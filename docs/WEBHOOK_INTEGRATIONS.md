...
Or put the settings in the project config:
...
Then check the configured Slack channel. If the run was
already notified, rotari will not send it again because it records
`webhook.sent` in the run directory.
...
Discord Webhooks and Teams Incoming Webhooks are not supported by a dedicated
format yet. Put an adapter such as n8n, Make, or Zapier between rotari and those
services.
