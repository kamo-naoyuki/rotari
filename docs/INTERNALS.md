...
- Web UI assets live under `cmd/rotari/assets/`: `web_template.html`,
    `web_styles.css`, and the ordered `web_app_*.js` files. `web.go` embeds
    them and concatenates the JavaScript files in this order:
    `core`, `actions`, `logs`, `tables`, `charts`, `bootstrap`. The browser
    still receives one script, so the files intentionally share the global
    scope; do not reorder them without checking the full Web test suite.
- The ordinary Web UI and the GitHub Pages static demo use the same JavaScript.
    `pageParts()` abstracts route parsing, `staticPath()` prefixes links for a
    repository base path, and `rewriteStaticLinks()` repairs dynamically
    created absolute links. Static export supplies `routeParts()` and replaces
    `fetch` for persisted state, logs, and reports before the app script runs.
    Keep the static bootstrap before the app script: otherwise the first state
    request can receive GitHub Pages' 404 HTML and briefly display it as text.
- Static pages receive a copy of `web_styles.css` beside each generated
    `index.html`; keep the stylesheet link relative so both `/` and a Pages
    subpath work. New static API endpoints need a corresponding response in
    the static bootstrap.
- Web template tests assert the final generated HTML contains required hooks
    and no obsolete queue vocabulary. Prefer adding stable semantic classes or
    data attributes over tests and UI logic that depend on formatted JS text or
    button labels.
- AI reports are generated by one Go formatter from persisted run state and
    bounded recent output. `show --report`, the Web UI's `/api/report`, and
    static Web generation use that formatter. Opening an AI service copies the
    report and opens a new tab; rotari does not transmit or submit the report.
